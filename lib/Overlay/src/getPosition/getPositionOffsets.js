...
function createEmptyArray(length, fn) {
  return Array.apply(undefined, _toConsumableArray(Array(length))).map(fn);
}
...
function normalizeOffset(offset, positionsLength) {
  ...
  if (typeof offset === 'number' || !Array.isArray(offset) && (typeof offset === 'undefined' ? 'undefined' : _typeof(offset)) === 'object') {
    ...
    if (offset.length < positionsLength) {
      ...
      normalizedOffsets = [].concat(_toConsumableArray(offset), _toConsumableArray(extraOffsets));
    } else {
...
function getPositionOffsets(positions, offset) {
  ...
  var positionOffsets = normalizedOffsets.map(function (normalizedOffset, index) {
    ...
    var positionMap = _positionsMap2.default[position];
    var offsetCorrection = positionMap.offset;
...
exports.default = getPositionOffsets;