...
describe('Accordion tab style props', function () {
  ...
  describe('tabStyle', function () {
    it('should apply tabStyle to all tabs', function () {
      expect(tabContentComponents.at(0).prop('wrapperStyle')).to.deep.equal(tabStyle);
      expect(tabContentComponents.at(1).prop('wrapperStyle')).to.deep.equal(tabStyle);
      expect(tabContentComponents.at(2).prop('wrapperStyle')).to.deep.equal(_extends({}, tabStyle, extraStyleProps));
    });
  });
