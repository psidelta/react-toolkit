...
describe('titlePosition', function () {
  ...
  describe('start', function () {
    it('renders title as first child', function () {
      ...
      expect(wrapper.find('.' + ROOT_CLASS + '__title-bar').childAt(0).props().className).to.equal(ROOT_CLASS + '__title-wrapper');
    });
  });
  ...
  describe('end', function () {
    it('renders title as last child', function () {
      ...
      expect(wrapper.find('.' + ROOT_CLASS + '__title-bar').childAt(1).props().className).to.equal(ROOT_CLASS + '__title-wrapper');
    });
  });
});