 ...
 * @return {Array}
 */
function computeXYDiff(xDiff, yDiff, handleX, handleY, handleName, aspectRatio, forceXDominate, forceYDominate) {
  if (handleName.length == 1) {
    ...
    if (handleX == handleY) {
      ...
      //top left
      if (handleX < 0) {
        if (forceYDominate !== true && (forceXDominate || xDiff - aspectRatio * yDiff < 0)) {
          //x dominates
          yDiff = xDiff / aspectRatio;
        } else {
          //y dominates
        ...
        //bottom right
        if (forceXDominate !== true && (forceYDominate || xDiff - aspectRatio * yDiff < 0)) {
          // y dominates
          xDiff = aspectRatio * yDiff;
        } else {
          // x dominates
