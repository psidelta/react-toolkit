...
describe('collapseDepth props', () => {
  describe('collapseDepth', () => {
    it('should nodes have a correct depth prop', () => {
      ...
      expect(wrapper.find(Node).first().props().depth).to.equal(0);
      expect(wrapper.find(Node).last().props().depth).to.equal(1);
    });
    ...
    it('should have no effect if selected is controlled or uncontrolled', () => {
      ...
      expect(wrapper.find(Node).first().props().collapsed).to.be.true;

      wrapper.setProps({ collapsed: { 1: true } });
      expect(wrapper.find(Node).at(1).props().collapsed).to.be.true;
    });
  });
