...
class ZippyComboBox extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div
        {...cleanProps(props, ZippyComboBox.propTypes)}
  ...
  setValue(newValue, { action } = {}) {
    ...
    if (
      ...
      (newValue && newValue.length > this.props.maxValueLength)
    ) {
      return null;
    }
  ...
  handleTagBackspaceRemove(event) {
    ...
    if (canDeleteTag) {
      ...
      if (this.areItemsGrouped) {
        ...
        const items = this.getSelectedItems();
        const lastItem = items[items.length - 1]; // value[value.length - 1]
