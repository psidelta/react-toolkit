...
class ZippyOverlay extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <div
        {...cleanProps(props, ZippyOverlay.propTypes)}
        className={className}
        ref={this.rootRef}
        style={style}
  ...
  getPositionConfig() {
    ...
    if (tooltipPositions) {
      positions = tooltipPositions
        .split(',')
        .map(
          position => (position && position.trim ? position.trim() : position)
        );
    }
